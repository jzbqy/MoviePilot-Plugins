...
| **自动开始** | 跳校完成后自动开始下载 | 开启 |
| **保留原分类** | 跳校后保留原分类设置 | 开启 |
| **删除导出的种子文件** | 跳校完成后删除导出的.torrent文件 | 开启（默认） |
...
| `*/2 * * * *` | 每2分钟执行一次 | 每2分钟 |
| `0 */6 * * *` | 每6小时执行一次 | 每6小时整点 |
| `0 0 */23 * *` | 每23小时执行一次 | 每23小时整点 |
| `0 2 * * *` | 每天凌晨2点执行 | 每天02:00 |
| `0 9 * * 1` | 每周一上午9点执行 | 每周一09:00 |
| `0 0 1 * *` | 每月1号执行 | 每月1日00:00 |

**注意**：cron表达式格式为 `分钟 小时 日 月 星期`，使用空格分隔
...
#### 🔴 数据完整性风险
- **文件损坏**：跳过校验可能导致已损坏的文件继续下载，造成数据不完整
- **部分文件缺失**：某些文件可能因为网络中断等原因未完全下载，跳校后无法发现
- **文件内容错误**：下载过程中可能出现的数据错误无法被检测到

#### 🔴 下载器稳定性问题
- **任务状态异常**：跳校后任务可能显示为"完成"但实际文件不完整
- **重复下载**：发现文件问题后需要重新下载，浪费带宽和时间
- **下载器崩溃**：某些情况下可能导致qBittorrent客户端不稳定

#### 🔴 网络和存储问题
- **带宽浪费**：如果文件已损坏，需要重新下载整个文件
- **存储空间浪费**：损坏的文件占用磁盘空间但无法正常使用
- **上传分享问题**：损坏的文件无法正常做种分享给其他用户

#### 🔴 站点和社区影响
- **做种质量下降**：上传损坏文件可能影响其他用户的下载体验
- **站点信誉影响**：长期上传损坏文件可能影响在PT站点的信誉
- **分享率问题**：损坏文件无法正常做种，可能影响分享率

#### 🔴 使用建议
- **谨慎使用**：仅在确认文件完整性或紧急情况下使用
- **定期检查**：跳校后建议手动检查重要文件的完整性
- **备份重要数据**：对重要文件建议先备份再跳校
- **监控下载状态**：跳校后密切关注下载任务的完成状态
...
### v1.0.4
- 优化cron表达式解析逻辑，支持更灵活的cron格式
- 默认开启"删除导出的种子文件"开关，节省磁盘空间
- 修复cron提示文本显示问题，如`*/2 * * * *`现在正确显示为"每2分钟执行一次"
- 改进cron表达式验证，支持`* * * * *`等标准格式
